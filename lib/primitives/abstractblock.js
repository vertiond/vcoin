...
const vertcoinpow = require('../crypto/vertcoinpow');
 ...
 * @returns {Hash|Buffer} hash
 */
AbstractBlock.prototype.powHash = function powHash(height, network) {
  var data = this.toHead();
  return vertcoinpow.pow(data, this.time, height, network);
}
...
AbstractBlock.prototype.verify = function verify(height, network) {
  if (!this.verifyPOW(height, network))
    return false;
...
AbstractBlock.prototype.verifyPOW = function verifyPOW(height, network) {
  return consensus.verifyPOW(this.powHash(height, network), this.bits);
};
