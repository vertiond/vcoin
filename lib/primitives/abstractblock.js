 ...
 * @returns {Hash|Buffer} hash
 */
AbstractBlock.prototype.powHash = function powHash(height) {
  var data = this.toHead();
  return vertcoinpow.pow(data, this.time, height);
}
...
AbstractBlock.prototype.verify = function verify(height) {
  if (!this.verifyPOW(height))
    return false;
...
AbstractBlock.prototype.verifyPOW = function verifyPOW(height) {
  return consensus.verifyPOW(this.powHash(height), this.bits);
};
