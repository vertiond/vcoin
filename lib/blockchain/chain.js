...
Chain.prototype.getTarget = async function getTarget(time, prev) {
  ...
  if ( /*testnet*/ false) {
    ...
    if (height % pow.retargetInterval !== 0) {
  ...
  return this.kimotoGravityWell(time, prev, spacing, blocksMin, blocksMax);
};
 ...
 * @param {Number} blocksMin - minimum blocks
 * @param {Number} blocksMax - maximum blocks
 */
Chain.prototype.kimotoGravityWell = async function kimotoGravityWell(time, prev, spacing, blocksMin, blocksMax) {
  ...
  for (let i = 1; reading && reading.height > 0; i++) {
    ...
    const eventHorizonDeviation = 1 + (0.7084 * pow(mass / 144, -1.228));
    const eventHorizonDeviationFast = eventHorizonDeviation;
    const eventHorizonDeviationSlow = 1 / eventHorizonDeviation;
