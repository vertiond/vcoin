...
Chain.prototype.verify = async function verify(block, prev, flags) {
  ...
  // reject version less than VERSION_TOP_BITS once segwit is enabled
  if (block.version < consensus.VERSION_TOP_BITS && state.hasWitness())
    throw new VerifyError(block, 'obsolete', 'bad-version', 0);
...
Chain.prototype.getDeployments = async function getDeployments(time, prev) {
  ...
  const nversionbips = await this.isActive(prev, deployments.nversionbips);

  // Coinbase heights are now enforced (bip34).
  if (height >= this.network.block.bip34height || nversionbips)
    state.bip34 = true;

  // Signature validation is now enforced (bip66).
  if (height >= this.network.block.bip66height || nversionbips)
    state.flags |= Script.flags.VERIFY_DERSIG;

  // CHECKLOCKTIMEVERIFY is now usable (bip65).
  if (height >= this.network.block.bip65height || nversionbips)
...
Chain.prototype._add = async function _add(block, flags, id) {
  ...
  if (flags & common.flags.VERIFY_POW) {
    if (!block.verifyPOW(prev.height + 1, this.network))
...
Chain.prototype.getTarget = async function getTarget(time, prev) {
  if (!prev) {
    return await this.getTargetBitcoin(time, prev);
  }

  const pow = this.network.pow;
  const height = prev ? prev.height + 1 : 0;

  // Vertcoin uses a different algorithm to calculate difficulty depending on block height
  if (this.network.type === 'testnet') {
    ...
    if (height >= 231000 && height < 231010) {
      return consensus.toCompact(pow.limit);
    }
    if (height % 12 !== 0) {
    ...
    if (height < 26754) {
      ...
      return 0x1e0ffff0;
    } else if (height >= 1080000 && height < 1080010) {
      return 0x1b0ffff0;
    } else if (height >= 1500000 && height < 1500010) {
      return 0x1c07fff8;
    }
  }

  const spacing = 2.5 * 60; // 2.5 minutes
  const dayInSeconds = 24 * 60 * 60;
  const blocksMin = (dayInSeconds * 0.25) / spacing;
  const blocksMax = (dayInSeconds * 7) / spacing;
  return await this.kimotoGravityWell(time, prev, spacing, blocksMin, blocksMax);
};

/**
 * Calculate the next target (using Bitcoin algorithm).
 * @param {Number} time - Next block timestamp.
 * @param {ChainEntry} prev - Previous entry.
 * @returns {Promise} - returns Number
 * (target is in compact/mantissa form).
 */
Chain.prototype.getTargetBitcoin = async function getTargetBitcoin(time, prev) {
  const pow = this.network.pow;
  ...
  // Back 2 weeks
  let back = pow.retargetInterval - 1;
...
/**
 * Calculate difficulty using kimoto gravity well formula.
 * @param {Number} time - Next block timestamp.
 * @param {ChainEntry} prev - Previous entry.
 * @param {Number} spacing - target block spacing in seconds.
 * @param {Number} blocksMin - minimum blocks to factor in.
 * @param {Number} blocksMax - maximum blocks to factor in.
 */
Chain.prototype.kimotoGravityWell = async function kimotoGravityWell(time, prev, spacing, blocksMin, blocksMax) {
  const pow = this.network.pow;

  if (!prev || prev.height === 0 || prev.height < blocksMin) {
    return pow.bits;
  }

  let reading = prev;
  let mass = 0;
  let rateActualSeconds = 0;
  let rateTargetSeconds = 0;
  let difficultyAverage = consensus.fromCompact(0);
  let difficultyAveragePrev = consensus.fromCompact(0);

  for (let i = 1; reading && reading.height > 0; i++) {
    if (blocksMax > 0 && i > blocksMax) {
      break;
    }

    mass++;
    if (i === 1) {
      difficultyAverage = consensus.fromCompact(reading.bits);
    } else {
      // difficultyAverage = (reading.bits - difficultyAveragePrev) / i + difficultyAveragePrev;
      difficultyAverage = consensus.fromCompact(reading.bits);
      difficultyAverage.isub(difficultyAveragePrev);
      difficultyAverage.idivn(i);
      difficultyAverage.iadd(difficultyAveragePrev);
    }
    difficultyAveragePrev = difficultyAverage;

    rateActualSeconds = prev.time - reading.time;
    rateTargetSeconds = spacing * mass;
    let rateAdjustmentRatio = 1.0;
    if (rateActualSeconds < 0) {
      rateActualSeconds = 0;
    }
    if (rateActualSeconds !== 0 && rateTargetSeconds !== 0) {
      rateAdjustmentRatio = rateTargetSeconds / rateActualSeconds;
    }

    const eventHorizonDeviation = 1.0 + (0.7084 * Math.pow(mass / 144.0, -1.228));
    const eventHorizonDeviationFast = eventHorizonDeviation;
    const eventHorizonDeviationSlow = 1.0 / eventHorizonDeviation;
    ...
    if (reading.prevBlock == null || (this.network.type === 'main' && (reading.height == 1080000 || reading.height == 1500000))) // Don't calculate past fork block on mainnet
    { 
          assert(reading); 
          break;
    }
    
    const cache = this.getPrevCache(reading);
    if (cache)
        reading = cache;
    else
        reading = await this.getPrevious(reading);

    assert(reading);
  }

  const target = difficultyAverage;
  if (rateActualSeconds !== 0 && rateTargetSeconds !== 0) {
    target.imuln(rateActualSeconds);
    target.idivn(rateTargetSeconds);
  }

  if(this.network.type === 'main') {
    // Code to activate mainnet fork
    if(prev.height+1 >= 1500000) {
        if (target.gt(pow.limit)) {
            return consensus.toCompact(pow.limit);
        }
    } else if (target.gt(pow.preVerthashLimit)) {
      return consensus.toCompact(pow.preVerthashLimit);
    }
  } else if(this.network.type === 'testnet') {
      if(prev.height+1 >= 231000) {
          if (target.gt(pow.limit)) {
              return consensus.toCompact(pow.limit);
          }
      } else if (target.gt(pow.preVerthashLimit)) {
...
Chain.prototype.retarget = function retarget(prev, first) {
  ...
  if (target.gt(pow.preVerthashLimit)) {
    return consensus.toCompact(pow.preVerthashLimit);
  }
